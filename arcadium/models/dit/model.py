...
from arcadium.models.dit.configuration import DiTConfig
...
class LabelEmbedder(nn.Module):
    """Embeds class labels into vector representations; handles label dropout for CFG."""
...
class DiT(PreTrainedModel):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        x: (N, C, H, W) spatial inputs (images or latents)
    ...
    def forward_with_cfg(self, x: torch.Tensor, t: torch.Tensor, y: torch.Tensor, cfg_scale: float) -> torch.Tensor:
        """Forward pass with classifier-free guidance, batching conditional and unconditional."""
        half = x[: len(x) // 2]
