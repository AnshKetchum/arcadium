...
class Attention(nn.Module):
    def __init__(self, d_model: int, d_head: int, n_query_heads, n_kv_heads: int, rope_base: int = 10000):
        super().__init__()
        ...
        self.q_norm = nn.RMSNorm(self.d_head * self.queries_per_group * self.n_kv_heads)
        self.k_norm = nn.RMSNorm(self.d_head * self.n_kv_heads)

        self.q_proj = nn.Linear(d_model, d_head * self.n_kv_heads * self.queries_per_group, bias=False)
        self.kv_proj = nn.Linear(d_model, 2 * d_head * self.n_kv_heads, bias=False)
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        # (B, queries_per_group, groups, T, d_head)
        out = einsum(rearrange(attn @ v, "(b q g t d) -> (b t q g d)"), "(b t q g d) -> (b t d)")
        return self.out(out)
...
class Block(nn.Module):
    def __init__(self, d_model: int, n_query_heads: int, n_kv_heads: int, mlp_expansion_factor = 4.0):
        super().__init__() 
        ...
        d_head = d_model // n_kv_heads
        self.attention = Attention(d_model, d_head, n_query_heads, n_kv_heads)
...
class Qwen3(PreTrainedModel):
    ...
    def __init__(self, config: Qwen3Config):
        ...
        self.blocks = nn.ModuleList([Block(config.d_model, config.n_query_heads, config.n_kv_heads)])
